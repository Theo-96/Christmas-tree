        ...
        if (!state.wasPinching) {
            ...
            if (intersects.length > 0) {
                ...
                if (target && target.userData.photoIndex !== undefined) {
                    ...
                    statusEl.innerText = " (" + (selectedIndex + 1) + "/" + state.photos.length + ")";
                }
            }
            ...
            if (selectedIndex !== -1) {
                ...
                statusEl.innerText = " (" + (state.currentPhotoIndex + 1) + "/" + state.photos.length + ")";
            }
...
function onResults(results) {
    ...
    if (results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
        ...
        if(debugEl) debugEl.innerText = `${state.isPinching ? '' : ''} | ${state.isHandOpen ? '' : ''}`;
    } else {
...
export function initMusic() {
    function initAudio() {
        ...
        bgMusic = new Audio('Christmas.mp3');
        bgMusic.volume = 0.5;
        bgMusic.loop = true;
